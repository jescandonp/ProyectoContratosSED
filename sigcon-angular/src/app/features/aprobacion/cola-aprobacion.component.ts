  ...
  template: `
    <div class="space-y-lg">
      <header class="flex flex-col gap-sm md:flex-row md:items-end md:justify-between">
        <div>
          <p class="m-0 text-xs font-bold uppercase tracking-wider text-[var(--color-primary)]">Aprobación de informes</p>
          <h1 class="m-0 mt-xs text-2xl font-bold text-[var(--color-on-surface)]">Cola de Aprobación</h1>
          <p class="m-0 mt-xs text-sm text-[var(--color-on-surface-variant)]">Informes en revisión asignados a su supervisión.</p>
        </div>
      ...
      <div class="rounded-xl border border-[var(--color-outline-variant)] bg-white overflow-hidden">
        @if (cargando()) {
          ...
          <div class="p-xl text-center text-sm text-[var(--color-on-surface-variant)]">No hay informes en revisión asignados a su supervisión.</div>
        } @else {
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-[var(--color-outline-variant)] bg-[var(--color-surface-container-low)]">
                  ...
                  <th class="px-md py-sm text-xs font-bold uppercase tracking-wider text-[var(--color-on-surface-variant)]">Último envío</th>
                ...
                @for (informe of page()!.content; track informe.id) {
                  <tr class="hover:bg-[var(--color-surface-container-low)] transition-colors">
                    ...
                    <td class="px-md py-sm">
                      <app-status-chip value="EN_REVISION" label="En revisión" tone="warning" />
                    </td>
...
export class ColaAprobacionComponent implements OnInit {
  ...
  cargar() {
    ...
    this.informeService.listarInformes({ page: this.paginaActual(), size: this.tamanioPagina }).subscribe({
      ...
      error: () => {
        this.error.set('No se pudo cargar la cola de aprobación.');
        this.cargando.set(false);
      }
    });
  }
  ...
  aprobar(informe: InformeResumen) {
